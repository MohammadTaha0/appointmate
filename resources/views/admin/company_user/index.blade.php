...
@section('main')
    ...
    <div class="table-responsive">
        <div class="input-group gap-1 mx-0 my-2">
            ...
            <div class="ms-auto search-cont col-md-8 col-11">
                <form
                    action="{{ route('admin.company_user.index', ['company_slug' => Str::slug(auth()->user()->getCompany->name ?? 'SA')]) }}"
                    class="input-group">
            ...
            <a href="{{ route('admin.company_user.create', ['company_slug' => Str::slug(auth()->user()->getCompany->name ?? 'SA')]) }}"
                ...
                <tr class="table-dark">
                    ...
                    <th width="6%">
                        @include('admin.components.th', [
                            'url' => route('admin.company_user.index', [
                            ...
                            <div class="dropdown">
                                ...
                                <ul class="dropdown-menu overflow-auto">
                                    <li><a class="dropdown-item d-flex justify-content-between align-items-center"
                                            href="{{ route('admin.company_user.edit', ['id' => $item->id, 'company_slug' => Str::slug(auth()->user()->getCompany->name ?? 'SA')]) }}">Edit
    ...
    @push('scripts')
        <script>
            updates(
                "{{ route('admin.company_user.updates', ['company_slug' => Str::slug(auth()->user()->getCompany->name ?? 'SA')]) }}");
            deletes(
                "{{ route('admin.company_user.delete', ['company_slug' => Str::slug(auth()->user()->getCompany->name ?? 'SA')]) }}");
            selectAll(
                "{{ route('admin.company_user.deleteAll', ['company_slug' => Str::slug(auth()->user()->getCompany->name ?? 'SA')]) }}"
                );
        </script>
    @endpush
@endsection
