...
@section('main')
    ...
    <div class="table-responsive">
        <div class="input-group gap-1 mx-0 my-2">
            ...
            <div class="ms-auto search-cont col-md-8 col-11">
                <form
                    action="{{ route('admin.company.index', ['company_slug' => Str::slug(auth()->user()->getCompany->name ?? 'SA')]) }}"
                    class="input-group">
            ...
            <a href="{{ route('admin.company.create', ['company_slug' => Str::slug(auth()->user()->getCompany->name ?? 'SA')]) }}"
                class="btn btn-dark"><i class="far fa-plus"></i>
                Create Another</a>
        </div>
                    ...
                    <tr>
                        ...
                        <td>
                            <a title="Edit" class="text-decoration-underline text-dark fw-bold"
                                href="{{ route('admin.company_user.edit', ['id' => $item->id, 'company_slug' => Str::slug(auth()->user()->getCompany->name ?? 'SA')]) }}">
                                {{ $item->id }}
                            </a>
                            <a target="_blank" title="View Appointment Page" class="btn btn-sm badge text-dark" href="{{ route('company.page', ['id' => $item->id, 'slug' => Str::slug($item->name)]) }}">
                        ...
                        <td>
                            <div class="input-group gap-1 gy-2 align-items-center">
                                @if ($item->image_exist)
                                    <img src="{{ $item->image_url }}" class="avatar-sm rounded-circle" alt="">
                                @endif
                                {{ $item->name }}
                            ...
                            <div class="dropdown">
                                ...
                                <ul class="dropdown-menu overflow-auto">
                                    <li><a class="dropdown-item d-flex justify-content-between align-items-center"
                                            href="{{ route('admin.company.edit', ['id' => $item->id, 'company_slug' => Str::slug(auth()->user()->getCompany->name ?? 'SA')]) }}">Edit
                                    ...
                                    <li><a class="dropdown-item d-flex justify-content-between align-items-center"
                                            href="{{ route('admin.company_user.create', ['company_slug' => Str::slug(auth()->user()->getCompany->name ?? 'SA')]) }}?company={{ $item->id }}">Create
                                            Admin
                                            <i class="fa-light fa-user-circle text-info"></i></a></li>

                                    <li><a class="dropdown-item d-flex justify-content-between align-items-center"
                                            href="{{ route('admin.company_user.index', ['company_slug' => Str::slug(auth()->user()->getCompany->name ?? 'SA')]) }}?key=company&value={{ $item->id }}">
                                            Admin's List
                                            <i class="fa-light fa-user-circle text-info"></i></a></li>
                                </ul>
                            </div>
    ...
    @push('scripts')
        <script>
            updates(
                "{{ route('admin.company.updates', ['company_slug' => Str::slug(auth()->user()->getCompany->name ?? 'SA')]) }}");
            deletes(
                "{{ route('admin.company.delete', ['company_slug' => Str::slug(auth()->user()->getCompany->name ?? 'SA')]) }}");
            selectAll(
                "{{ route('admin.company.deleteAll', ['company_slug' => Str::slug(auth()->user()->getCompany->name ?? 'SA')]) }}"
                );
        </script>
    @endpush
@endsection
