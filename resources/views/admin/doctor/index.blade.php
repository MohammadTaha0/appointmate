...
@push('second_bread', 'Doctors Lists')
@section('main')
    ...
    <div class="table-responsive">
        <div class="input-group gap-1 mx-0 my-2">
            ...
            <div class="ms-auto search-cont col-md-8 col-11">
                <form
                    action="{{ route('admin.doctor.index', ['company_slug' => Str::slug(auth()->user()->getCompany->name ?? 'SA')]) }}"
                    class="input-group">
            ...
            <a href="{{ route('admin.doctor.create', ['company_slug' => Str::slug(auth()->user()->getCompany->name ?? 'SA')]) }}"
                    ...
                    <tr>
                        ...
                        <td>
                            <a title="Edit" class="text-decoration-underline text-dark fw-bold"
                                href="{{ route('admin.doctor.edit', ['id' => $item->id, 'company_slug' => Str::slug(auth()->user()->getCompany->name ?? 'SA')]) }}">
                                {{ $item->id }}
                            </a>
                        </td>
                        @can('SA')
                            ...
                            <div class="dropdown">
                                ...
                                <ul class="dropdown-menu overflow-auto">
                                    <li><a class="dropdown-item d-flex justify-content-between align-items-center"
                                            href="{{ route('admin.doctor.edit', ['id' => $item->id, 'company_slug' => Str::slug(auth()->user()->getCompany->name ?? 'SA')]) }}">Edit
    ...
    @push('scripts')
        <script>
            updates(
                "{{ route('admin.doctor.updates', ['company_slug' => Str::slug(auth()->user()->getCompany->name ?? 'SA')]) }}"
                );
            deletes(
                "{{ route('admin.doctor.delete', ['company_slug' => Str::slug(auth()->user()->getCompany->name ?? 'SA')]) }}"
                );
            selectAll(
                "{{ route('admin.doctor.deleteAll', ['company_slug' => Str::slug(auth()->user()->getCompany->name ?? 'SA')]) }}"
            );
        </script>
    @endpush
@endsection
